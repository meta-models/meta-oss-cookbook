...
The diagram shows the shape on a single workstation GPU. The same wiring applies to the B300 deployment below, with the serving layer on the data-center card instead.
...
These are **single-GPU** numbers: the host was an 8 × B300 node, but bf16 fits in one 288 GB card, so the server ran without tensor parallelism. Scale accordingly if you plan to shard.
